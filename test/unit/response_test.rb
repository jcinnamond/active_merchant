...
class ResponseTest < Test::Unit::TestCase
  ...
  def test_three_d_secure_redirect_url
    term_url = 'http://website.com/payment/process_three_d_secure'
    pa_req ='eJxVUttygjAQfe9XMH4AuUCoOGscW9'
    md = '2012354765399251503'
    acs_url = 'https://ukvpstest.protx.com/mpitools/accesscontroler?action=pareq'
    response = Response.new(false, 'message', {}, :three_d_secure => true, :pa_req => pa_req, :md => md, :acs_url => acs_url)
    
    assert_equal "#{acs_url}&MD=#{CGI.escape(md)}&PaReq=#{CGI.escape(pa_req)}&TermUrl=#{CGI.escape(term_url)}",
      response.three_d_secure_redirect_url(term_url)
  end
